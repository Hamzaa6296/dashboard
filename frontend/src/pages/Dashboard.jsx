...
import { FolderKanban, Users, CheckCircle, TrendingUp, Clock, Plus } from 'lucide-react';
...
function StatCard({ icon: Icon, label, value, color }) {
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontSize: 28, fontFamily: 'Syne', fontWeight: 700, color }}>{value}</div>
...
function ActivityItem({ activity }) {
  const actionColors = {
    created: 'var(--green)', updated: 'var(--accent)',
    deleted: 'var(--red)', joined: 'var(--yellow)', logged_in: 'var(--text-3)',
  };
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <span style={{ fontSize: 14 }}>
          ...
          {activity.resource_title && (
            <span style={{ color: 'var(--text-2)' }}>{activity.resource_title}</span>
          )}
        </span>
      </div>
      <span style={{ fontSize: 12, color: 'var(--text-3)', flexShrink: 0 }}>{time}</span>
    </div>
  );
}
  ...
  return (
    <div className="fade-in" style={{ maxWidth: 1200 }}>
      ...
      <div className="page-header">
        <div>
          <h1 style={{ fontFamily: 'Syne', fontSize: 'clamp(22px, 4vw, 28px)', fontWeight: 700, marginBottom: 4 }}>
      ...
      <div className="stats-grid">
        <StatCard icon={FolderKanban} label="Total Projects" value={stats.totalProjects ?? 0} color="var(--accent)" />
        <StatCard icon={TrendingUp} label="Active" value={stats.activeProjects ?? 0} color="var(--green)" />
        <StatCard icon={CheckCircle} label="Completed" value={stats.completedProjects ?? 0} color="var(--yellow)" />
        <StatCard icon={Users} label="Team Members" value={stats.totalUsers ?? 0} color="var(--accent-2)" />
      </div>
      ...
      <div className="charts-grid">
        ...
        <div style={{
          ...
          <h3 style={{ fontFamily: 'Syne', fontSize: 16, fontWeight: 600, marginBottom: 20 }}>
          ...
          {data?.projectTrend?.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={data.projectTrend} barSize={28}>
                ...
                <Tooltip contentStyle={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)' }} cursor={{ fill: 'var(--bg-3)' }} />
          ...
          {data?.projectsByStatus?.length > 0 ? (
            <>
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie data={data.projectsByStatus} dataKey="count" nameKey="status" cx="50%" cy="50%" innerRadius={45} outerRadius={70}>
                ...
                {data.projectsByStatus.map((entry, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 13 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ color: 'var(--text-2)', textTransform: 'capitalize' }}>{entry.status}</span>
                    </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3 style={{ fontFamily: 'Syne', fontSize: 16, fontWeight: 600 }}>Recent Activity</h3>
          <Clock size={16} color="var(--text-3)" />
        </div>
        {data?.recentActivities?.length > 0 ? (
          data.recentActivities.map(a => <ActivityItem key={a.id} activity={a} />)
        ) : (
